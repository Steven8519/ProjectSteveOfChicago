...
export class RegisterComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.authService.registerUser(user).subscribe(data => {
      if (data.success) {
        this.flashMessage.show("Registration Successful: ", {cssClass: "alert-success", timeout: 6000 });
        this.router.navigate(["/login"]);
      } else {
        this.flashMessage.show("Registration was unsuccessful: ", {cssClass: "alert-danger", timeout: 6000 });
